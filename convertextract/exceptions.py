...
class CorrespondenceMissing(CommandLineError):
    ...
    def __str__(self):
        return self.render((
            'There is no mapping with the name "%(language)s", please\n'
            'make sure you spelled the name correctly or go to\n'
            'https://github.com/roedoejet/convertextract/ for a list of mappings'
        ))
...
class MalformedCorrespondence(CommandLineError):
    ...
    def __str__(self):
        return self.render((
            'Your mapping with the name "%(mapping)s", is malformed.\n'
            'Do all of your mappings have values for "in" and "out"?\n'
            'Please fix your lookup table.'
        ))
...
class ExtensionNotSupported(CommandLineError):
    ...
    def __init__(self, ext):
        ...
        from convertextract.parsers import _get_available_extensions
